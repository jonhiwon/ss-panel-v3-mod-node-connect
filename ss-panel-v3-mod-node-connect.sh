...
install_centos_ssr(){
	...
	ldconfig
	git clone -b manyuser https://github.com/jonhiwon/shadowsocks.git "/root/shadowsocks"
	cd /root/shadowsocks
	...
	if [ -z "`python -c 'import requests;print(requests)'`" ]; then
		...
		git clone https://github.com/jonhiwon/urllib3.git && cd urllib3
		python setup.py install && cd ..
		git clone https://github.com/jonhiwon/CyMySQL.git && cd CyMySQL
		python setup.py install && cd ..
		git clone https://github.com/jonhiwon/requests.git && cd requests
		python setup.py install && cd ..
		git clone https://github.com/jonhiwon/pyopenssl.git && cd pyopenssl
		python setup.py install && cd ..
		git clone https://github.com/jonhiwon/ndg_httpsclient.git && cd ndg_httpsclient
		python setup.py install && cd ..
		git clone https://github.com/jonhiwon/pyasn1.git && cd pyasn1
		python setup.py install && cd ..
		rm -rf python
	fi	
...
install_ubuntu_ssr(){
	...
	pip install cymysql
	cd /root
	git clone -b master https://github.com/jonhiwon/shadowsocks.git "/root/shadowsocks"
	cd shadowsocks
...
install_node(){
	...
	echo "# Github: https://github.com/jonhiwon   "
	echo "# Author: Ttnu                                                    "
	echo "# blog: https://www.ttnu.cn                                      "	
	echo "#########################################################################"
	echo
	#Check Root
	[ $(id -u) != "0" ] && { echo "Error: 必须使用root用户执行此脚本！"; exit 1; }
	...
	read -p "请输入面板的域名或ip(例如:https://www.ttnu.cn or http://114.114.114.114): " Userdomain
	read -p "请输入面板的muKey(例如:mupass): " Usermukey
	read -p "请输入面板的节点id(例如:1): " UserNODE_ID
	install_ssr_for_each
	cd /root/shadowsocks
...
install_node_db(){
	...
	supervisorctl shutdown
	#某些机器没有echo_supervisord_conf 
	wget -N -P  /etc/ --no-check-certificate  https://raw.githubusercontent.com/jonhiwon/ss-panel-v3-mod-node-connect/master/supervisord.conf
	supervisord
	#iptables
	iptables -F
	iptables -X  
